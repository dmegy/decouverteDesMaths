...
(La définition donnée est insuffisante car en réalité, toutes les collections ne sont pas autorisées (pour éviter certains paradoxes). Mais dans la plupart de celles auxquelles on peut penser forment bien des ensembles au sens mathématique du terme). 
...
Attention, dans une définition par énumération, il n'y a pas de notion d'ordre, ni de multiplicité (un élément ne peut pas appartenir \og plusieurs fois\fg{} à un ensemble). Donc  $\left\{1,3,\pi,5,\sqrt2\right\} = \left\{\sqrt 2, 3,5,1,\pi\right\}=\left\{\sqrt 2,\sqrt 2, 3,  3,5,1,1,\pi\right\}$.

\begin{definition}(Ensemble vide, singleton, paire)
\index{ensemble!vide}\index{vide!ensemble}\index{singleton}
\begin{enumerate}
...
C'est le paramétrage standard de cet intervalle. Lorsque $t=0$, le réel $a+t(b-a)$ vaut $a$, lorsque $t=1$, le réel $a+t(b-a)$ vaut $b$, et lorsque $t$ varie entre $0$ et $1$, le réel $a+t(b-a)$, varie entre $a$ et $b$.
...
\begin{exercice}[Parties définissables]
On dit qu'une partie $A$ de $\R$ est \emph{définissable} si on peut l'écrire en compréhension sous la forme
\[ A=\{x\in \R\:|\: \Phi(x)\}\]
où $\Phi(x)$ est équivalente à une \og formule admissible\fg, c'est-à-dire une formule du premier ordre avec des quantificateurs portant obligatoirement sur $\R$, et faisant intervenir des constantes réelles, la somme de réels et le produit de réels, le symbole $\in$, les symboles logiques \og et\fg, \og ou\fg{} et la négation \og non\fg.

On autorise donc également les symboles $\cup$, $\cap$, complémentaire, puisqu'ils peuvent s'écrire avec les opérateurs logiques décrits plus haut.

\begin{enumerate}
\item Montrer que $\R_+$ est définissable, ainsi que $\R_+^*$.
\item Montrer que tout intervalle de $\R$ est définissable.
\item Montrer que la partie $\ensemble{x\in\R}{\Phi(x)}$, où $\Phi(x)$ est la formule $\exists y\in [2,3[, x\leq y^2+1-3y$, est définissable, en exhibant une formule équivalente admissible.
\end{enumerate}
Remarque : le théorème de Tarski-Seidenberg énonce que les seules parties définissables de $\R$ sont les unions finies d'intervalles. Sa preuve est très difficile. En particulier, la partie $\N \subset \R$ n'est \textbf{pas} définissable. ($\Z$ et $\Q$ non plus.)
\end{exercice}
