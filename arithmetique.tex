...
Soit $\alpha$ un entier relatif. On note $\alpha\Z$ l'ensemble $\{ \alpha k\:\mid\: k\in \Z\} = \{..., -2\alpha, -\alpha, 0, \alpha, 2\alpha, 3\alpha, ...\}$. C'est l'ensemble des multiples de $\alpha$. Les ensembles $\alpha\Z$ et $(-\alpha)\Z$ sont identiques.

Le \emph{générateur principal} de $\alpha\Z$ est $|\alpha|$.
\end{definition}

Exemples : $3\Z = \{...,-6,-3,0,3,6,9,12,...\}$. Si $\alpha=0$, alors $\alpha\Z = \{0\}$. On a $\alpha\Z = \Z$ ssi $\alpha$ est égal à $1$ ou $-1$. Plus généralement, on a $a\Z=b\Z$ ssi $a=b$ ou $a=-b$.
...
\begin{proposition}
Soient $a$ et $b$ deux entiers. L'ensemble $\{ak+bl\:\mid\: k, l\in \Z\}$ noté par définition $a\Z+b\Z$, est un idéal de $\Z$.
C'est le plus petit idéal de $\Z$ contenant $a$ et $b$. % utile pour les preuves
\end{proposition}
\begin{proof}
Il suffit de vérifier que c'est un sous-groupe de $\Z$.
\end{proof}
...
\begin{theoreme}[\og théorème/lemme de Gau{\ss}\fg]
