...
Attention, la présentation qui suit diffère sans doute beaucoup de celle vue en terminale : il faut faire l'effort de l'étudier en détail même si l'ordre dans lequel les notions sont introduites semble inhabituel. En particulier
...
\begin{proposition}[Division euclidienne]\index{division euclidienne}
Soit $a\in \N$ et $b\in \N^*$. Il existe un unique couple $(q,r) \in \N^2$ vérifiant les deux propriétés suivantes:
\begin{enumerate}
\item $a=bq+r$;
\item $r < b$.
\end{enumerate}
L'entier $q$ est le \emph{quotient} de la division euclidienne de $a$ par $b$, et $r$ est le \emph{reste}.
Effectuer la division euclidienne de $a$ par $b$, c'est écrire $a = bq+r$ avec $q$ et $r$ comme plus haut.
\end{proposition}
...
\begin{definition}[Ensembles $\alpha\Z$ et générateur principal]\index{$\alpha\Z$}
...
\item Le \emph{générateur principal}\index{générateur principal} de $\alpha\Z$ est $|\alpha|$.
\end{enumerate}
\end{definition}
...
\begin{definition}\index{sous-groupe de $\Z$}
...
\item L'entier $d$ est dans $d\Z$, donc il existe $k$ et $l$ dans $\Z$, tels que $d = ak+bl$. On dit que $(k,l)$ est un couple (ou paire, par abus de langage) de Bézout pour $a$ et $b$. L'égalité $d=ak+bl$ est appelée \emph{relation de Bézout}\index{Bézout!relation}.
...
\end{proposition}
\begin{proof}
Si $x|a$ et $x|b$, alors $x|d$. D'autre part, $d|a$ et $d|b$, donc $d|x$. Donc finalement, $d=x$.
...
\begin{lemme}[d'Euclide]\index{Euclide!lemme}
...
\begin{theoreme}[Algorithme d'Euclide]\index{Euclide!algorithme}
...
\begin{definition}\index{nombres premiers entre eux}
Deux nombres relatifs $a$ et $b$ sont premiers entre eux si $\pgcd(a,b)=1$. On note :  $a\wedge b = 1$. (Ou parfois $a\bot b$ : c'est une bonne notation mais on ne l'utilisera pas ici.)
\end{definition}
...
\begin{theoreme}[\og théorème de Gau{\ss}\fg]\index{Gau\ss!théorème}
...
\begin{definition}\index{équation diophantienne}
...
\begin{proposition}\index{équation diophantienne!homogène}
...
\begin{definition}\index{nombres premiers}
...
\begin{definition}\index{nombres composés}
...
\begin{proposition}[Test de primalité]\index{Test de primalité}
Un entier $n\geq 2$ est premier si $\forall a\in\N,  2\leq a\leq \sqrt n \implies a\nmid n$.
\end{proposition}
\begin{proof}
Si $n$ est composé, alors $n=ab$ avec $a, b \in \llbracket 2,n-1 \rrbracket$, donc au moins un des deux entiers $a$ ou $b$ est $\leq\sqrt n$ (sinon on aurait $n=ab > \sqrt{n}^2=n$, absurde).
\end{proof}
...
\begin{proposition}
\index{nombres premiers!décomposition en facteurs premiers}\index{factorisation!en produit de nombres premiers}
\[
...
\textbf{Hérédité sous hypothèse de récurrence forte.} Soit $n\geq 1$, et supposons $A(k)$ vraie pour tout $k\leq n$. Montrons $A(n+1)$. Si $n+1$ est premier, alors la suite $\alpha_{n+1}=1$ et $\alpha_i=0$ pour $i\neq n+1$ convient. Si $n+1$ est composé, écrivons $n+1=bc$ avec $b,c\leq n$. Par hypothèse de récurrence appliquée à $b$ et $c$, on peut écrire $b=\prod_{p\in \mathcal P} p^{\beta_p}$ et $c=\prod_{p\in \mathcal P} p^{\gamma_p}$. On a donc 
...
\begin{definition}[Valuation $p$-adique]\index{valuation $p$-adique}
...
\begin{proposition}\index{nombres premiers!infinitude des}
L'ensemble $\mathcal P$ des nombres premiers est infini.
\end{proposition}
\begin{proof}
Supposons par l'absurde qu'il soit fini, et soit $M$ son plus grand élément. Pour tout $k\leq M$, $k$ divise $M!$, donc si $k>1$, le reste par la division euclidienne de $M!+1$ par $k$ est $1$. On en déduit que $k$ ne divise pas $M!+1$. En particulier, aucun nombre premier ne divise $M!+1$, qui ne possède donc pas de décomposition en facteurs premiers, absurde.
\end{proof}

Remarque : ceci n'est pas la preuve d'Euclide, qui est une preuve directe et non une preuve par l'absurde. Euclide montre qu'étant donné un ensemble fini de nombres premiers, il existe un nombre premier n'appartenant pas à cet ensemble.

\section{Compléments}

Le petit théorème de Fermat, le théorème chinois et l'indicatrice d'Euler sont traités au second semestre.