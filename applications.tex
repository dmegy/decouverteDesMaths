...
L'ensemble de toutes les permutations de $E$ est noté $\Bij(E)$, ou encore $\mathfrak S_E$ ($S$  majuscule gothique, en police \emph{Fraktur}).
...
\begin{exemple}\label{ex-involution-1-x}
...
\begin{exercice}\label{ex-involution-cubique}
Dans cet exercice on note $\sqrt[3]{}$ la racine cubique définie sur $\R$.
Montrer que les fonctions $g : \R\to \R, x\mapsto \sqrt[3]{1-x^3}$ est $h : \R\to \R, x\mapsto (1-\sqrt[3]{x})^3$ sont involutives . 
\end{exercice}
...
\index{conjugaison!par une bijection}
...
\begin{enumerate}
\item Montrer que si $f$ est une involution, alors $g\circ f \circ g^{-1}$ aussi. Quel est le rapport avec les exercices \ref{ex-involution-1-x} et \ref{ex-involution-cubique} (écrire les fonctions sous forme de composées) ?
