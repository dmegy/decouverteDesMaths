...
L'image d'une application est donc une partie (stricte ou pas) de son codomaine.
...
\begin{exercice}[Coordonnées polaires sur le plan]
Montrer que 
...
Montrer que les fonctions $g : \R\to \R, x\mapsto \sqrt[3]{1-x^3}$ est $h : \R\to \R, x\mapsto (1-\sqrt[3]{x})^3$ sont involutives . Quel est le rapport avec la fonction $f$ de l'exemple \ref{ex-involution} (écrire les fonctions sous forme de composées) ?\footnote{Ce procédé s'appelle une conjugaison. On rencontrera souvent ce type de situation, en particulier en algèbre linéaire.}
% plus généralement, si $\phi$ est une bijection, mq la conjuguée d'une involution est une involution ?
\end{exercice}

% \subsection{Conjugaison par une bijection}
% ici mettre des diagrammes, le fait que conjuguer par une bijection conserve le caractère involutif, bouge les points fixes, bouge les parties stables etc etc.
