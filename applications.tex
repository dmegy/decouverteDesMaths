...
Montrer que les fonctions $g : \R\to \R, x\mapsto \sqrt[3]{1-x^3}$ est $h : \R\to \R, x\mapsto (1-\sqrt[3]{x})^3$ sont involutives. (On peut généraliser cette construction, voir l'exercice \ref{exo-conjugaison}.)
...
\draw[smooth,samples=100,domain=-3:1,thick,blue] plot(\x,{(1-(\x)^3)^(1/3)});
\draw[smooth,samples=100,domain=1:3,thick,blue] plot(\x,{-(-1+(\x)^3)^(1/3)});
\draw[dashed] (-3,-3) -- (3,3);
\end{tikzpicture}
\hfill
