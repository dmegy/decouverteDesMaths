...
\begin{exemple}
Un élément du codomaine peut ne pas avoir d'antécédents, ou en avoir plusieurs. Par exemple, si $f : \R\to \R, x\mapsto x^2$, alors l'élément $-1$ n'a aucun antécédent (il n'existe pas de $x\in \R$ tel que $f(x)=x^2=-1$). L'élément $0$ a exactement un antécédent ($0$), et l'élément $4$ a deux antécédents : $2$ et $-2$.
\end{exemple}
...
\textbf{Attention}, on voit très souvent la notation $f^{-1}(B)$ au lieu de $f^{<-1>}(B)$ mais cela peut prêter à confusion, la fonction $f$ n'ayant pas forcément de réciproque. Dans ce chapitre, on utilise pour commencer la notation $f^{<-1>}(B)$. La notation $f^{-1}(B)$ sera adoptée progressivement dans les chapitres suivants.
\end{definition}
...
\end{exemple}
\begin{red}
Soient $x, y\in \R$. Supposons $f(x)=f(y)$ c'est-à-dire $x^2=y^2$. Alors, $x=y$ ou $x=-y$, et donc on en déduit que $|x|=|y|$ c'est-à-dire $g(x)=g(y)$. D'après la proposition \ref{factorisation-droite}, il existe $h : \R\to \R$ telle que $g =h\circ f$, ou encore : $\forall x\in \R, |x|=h(x^2)$. 
\end{red}
...
\begin{exercice}[Union disjointe/coproduit/somme cartésienne]\index{union disjointe}\index{propriété universelle!de l'union disjointe}
...
Montrer que $A\biguplus B$ ainsi que $i_A$ et $i_B$ vérifient la propriété suivante (dite \emph{propriété universelle du coproduit}): 
\begin{quote}
Pour tout ensemble  $C$ muni d'applications $f : A\to C$ et $g : B\to C$, il existe une unique application $h : A\biguplus B \to C$ telle que $f = h\circ i_A$ et $g = h\circ i_B$, autrement dit il existe une unique application h faisant commuter le diagramme  suivant:
\[ 
\xymatrix{
...
\begin{exercice}[Égalisateurs]\index{égalisateur}\index{propriété universelle!de l'égalisateur}
...
\begin{exercice}[Produit fibré]\index{propriété universelle!du produit fibré}\index{produit fibré d'ensembles}
