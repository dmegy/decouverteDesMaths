...
\end{enumerate}
Ces assertions sont vraies si et seulement si $f(A)\subseteq g_*(B)$.
\end{proposition}
\begin{proof}
\textbf{Sens \og seulement si\fg.} Supposons $f = g\circ h$, et soit $a\in A$. Alors $f(a) = g(h(a))$, donc $f(a) \in g_*(B)$. Ceci prouve que $f(A)\subseteq g_*(B)$.\\
\textbf{Sens \og si\fg.} Supposons $f_*(A)\subseteq g_*(B)$. Construisons une application $h$ vérifiant les conditions demandées. Soit $a\in A$. Comme $f(a) \in f(A) \subseteq g_*(B)$, $f(a) \in g_*(B)$ et donc $f(a)$ possède un antécédent par $g$. On définit $h(a)$ comme étant un tel antécédent. Par construction, on a $g\circ h = f$.
\end{proof}
...
\end{propdef}
\begin{proof}
Par définition de $B$, on a $B = \{x\in E\:|\: f(x)\in f(A) \}$ et donc par définition d'une image réciproque, $B = f^*(f_*(A))$.
\end{proof}
