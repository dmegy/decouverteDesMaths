...
Une relation binaire $R$ sur un ensemble $E$ est une \emph{relation d'équivalence} ssi elle est:
\begin{enumerate}
\item réflexive (rappel : $\forall x\in E, xRx$);
\item transitive (rappel : $forallx, y, z\in E, xRy\text{ et } yRz \implies xRz$);
\item symétrique : $\forall x, y\in E, xRy \implies yRx$.
\end{enumerate}
\end{definition}
...
\begin{definition}
Soit $E$ un ensemble muni d'une relation d'équivalence $R$. Soit $x\in E$. On note $\overline{x}$ et on appelle la \emph{classe d'équivalence} de $x$ l'ensemble $\{y\in E\:\mid\: yRx\}$ des éléments qui sont équivalents à $x$.
\end{definition}