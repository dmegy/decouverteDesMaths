...
\item transitive\index{relation!transitive} ssi $\forall x, y, z\in E, x\mathcal Ry \text{ et } y{\mathcal R}z \implies x{\mathcal R}z$;
...
\item symétrique (rappel : $\forall x, y\in E, x{\mathcal R}y \implies y{\mathcal R}x$).
\end{enumerate}
\end{definition}
...
\item L'ensemble vide\index{vide!ensemble} possède une seule relation d'équivalence, la relation vide (la seule fonction de $\varnothing \times \varnothing$ dans $\{\text{vrai,\: faux}\}$ à savoir la fonction vide : on vérifie qu'elle définit bien une relation d'équivalence).
...
\begin{proof}
Soit $x \in E$.
Alors on a bien $f(x)=f(x)$ donc $x\mathcal R x$, donc $\mathcal R$ est réflexive.
Si $x, y\in E$, on a bien $x\mathcal R y \iff f(x)=f(y) \iff f(y)=f(x) \iff y\mathcal R x$ donc $\mathcal R$ est symétrique.
Et enfin, si $x,y,z\in E$ et que $x\mathcal R y$ et $y\mathcal R z$, alors $f(x)=f(y)$ et $f(y)=f(z)$, donc $f(x)=f(z)$ et donc $x\mathcal R z$, donc $\mathcal R$ est transitive.
Ceci montre que $\mathcal R$ est bien une relation d'équivalence.
...
\item $\pi/3 \equiv 13\pi/3 \modulo{2\pi}$, car $\pi/3 - 13\pi/3 = -12\pi/3 = -4\pi$ est un multiple de $2\pi$.
...
Sens $\implies$ : Soit $z\in [x]$. Alors $z{\mathcal R}x$ et comme $x{\mathcal R}y$, on a $z{\mathcal R}y$ par transitivité, et donc $z\in [y]$. Ceci montre $[x]\subseteq [y]$. Pour montrer l'inclusion réciproque, on a $y{\mathcal R}x$ par symétrie de $\mathcal R$ puis on termine de la même manière.
...
\begin{exemple}
\begin{enumerate}
\item L'ensemble vide\index{vide!ensemble} possède une seule partition, la partition vide qui ne contient aucune partie (car les parties elles, doivent être non-vides).
...
Soit $E$ un ensemble muni de deux relations d'équivalence $\mathcal R$ et $\mathcal S$, de graphes $\Gamma_{\mathcal R}$ et $\Gamma_{\mathcal S}$. L'ensemble $\Gamma_{\mathcal R} \cap \Gamma_{\mathcal S} \subseteq E\times E$ est le graphe d'une relation d'équivalence sur $E$. 
...
\begin{exercice}[Union/disjonction et intersection/conjonction de deux relations]\label{exo-union-intersection-relations}
...
\begin{exercice}[Somme / union amalgamée]
