...
\begin{exemple}
Sur un ensemble fini $E$  de cardinal $n$, il existe $2^{(n^2)}$ relations binaires différentes.
\end{exemple} 
...
Attention, contrairement à ce qu'on peut croire, une relation peut être à la fois symétrique et antisymétrique (exemple : l'égalité), même si c'est assez rare.

Le tableau suivant résume quelques propriétés des relations classiques. (Certains points seront repris par la suite.)\\
...
\begin{exercice}
Montrer que si une relation $\mathcal R$ est réflexive (resp. symétrique, transitive), alors ${}^t\mathcal R$ aussi.

Montrer que $\mathcal R$ est symétrique si et seulement si $\mathcal R = {}^t\mathcal R$.
\end{exercice}
...
Un ensemble $E$ muni d'une relation d'ordre ${\mathcal R}$ est appelé \emph{ensemble ordonné}\index{ensemble ordonné}. Par exemple, $(\R, \leq)$ est un ensemble ordonné.
...
Enfin, les notations $x\geq_E y$ et $x >_E y$ sont synonymes de $y \leq_E x$ et $y <_E x$.
...
\index{partie saturée!sous une relation d'équivalence}
...
% raffinement d'une relation d'équivalence : relation d'ordre sur les relations d'équivalence
% "relation d'ordre la moins fine qui" ...
